...
export const deleteMatchGroupAsset = functions.firestore.document(userPath).onUpdate(async (change, _context) => {
  ...
  const removedGroupAssetIDs = difference(userBefore.matchGroupAssetIDs ?? [], userAfter.matchGroupAssetIDs ?? [])

  if (removedGroupAssetIDs.length === 0) {
    return { message: 'There are no new matchGroupAssetID to delete.', contents: null }
  }

  const task = removedGroupAssetIDs.map(async removedGroupAssetID => {
    const groupAssetsRef = change.after.ref.collection('matchGroupAssets')
