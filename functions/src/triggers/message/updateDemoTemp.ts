...
const expo = new Expo()
const messagePath = 'parties/{partyID}/messages/hd27FtjcIC5jxsMmRfnU'

export const updateDemoTemp = functions.firestore.document(messagePath).onUpdate(async (snap, context) => {
  ...
  const message = buildMessage(snap.after.data()!)
  ...
  const result = {
    documentID: snap.after.ref,
    path: snap.after.ref.path,
    value: message
  }
